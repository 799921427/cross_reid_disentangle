...
from reid.models.resnext import resnext101_ibn_a
...
def load_pretrain_param(model, model_path):
    param_dict = torch.load(model_path)["state_dict"]
    for i in param_dict:
        j = i[7:] if i[:6] == "module" else i
        if 'fc' in j:
            continue
        if j not in model.state_dict().keys():
            print("missing key: {}".format(j))
...
class ft_netAB(nn.Module):
    ...
    def __init__(self, class_num, norm=False, stride=2, droprate=0.5, pool='avg'):
        ...
        # load_pretrain_param(model_ft, model_path)
        self.part = 4
        if pool=='max':
            model_ft.partpool = nn.AdaptiveMaxPool2d((self.part,1))
            model_ft.avgpool = nn.AdaptiveMaxPool2d((1,1))
