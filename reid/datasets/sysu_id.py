...
class SYSU_ID(Dataset):
    def __init__(self, root, split_id=0, num_val=100, download=True):
        super(SYSU_ID, self).__init__(root, split_id=split_id)
    ...
    def download(self):
        ...
        identities = [[[] for _ in range(4)] for _ in range(533)]

        for pid in range(1, 534):
            for cam in range(1,5):
                if cam > 2: images_path = self.root+"/cam"+str(cam+1)+"/"+str(pid).zfill(4)
                else : images_path = self.root+"/cam"+str(cam)+"/"+str(pid).zfill(4)
        ...
        meta = {'name': 'sysu', 'shot': 'multiple', 'num_cameras': 4,
