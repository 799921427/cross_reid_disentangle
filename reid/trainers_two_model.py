...
class BaseTrainer(object):
    def __init__(self, model_s, model_t, criterion_z, criterion_z_s, criterion_I, criterion_I_s, criterion_att, trainvallabel, a, b, c, u, k):
        ...
        self.criterion_I_s = criterion_I_s
    ...
    def train(self, epoch, data_loader, optimizer_generator_I, print_freq=1):
        ...
        for i, inputs in enumerate(data_loader):
            ...
            L = self.a * loss_t + self.b * loss_id + loss_attention
            # L = loss_attention
...
class Trainer(BaseTrainer):
    ...
    #loss_id = loss_id + 0.1 * self.criterion_z(att_cls, label)
        ...
        outputs, outputs_pool, att_feats, g_att = self.model_t(att_inputs)
        outputs_s, outputs_pool_s, att_feats_s, g_att_s = self.model_s(att_inputs)
        loss_t_s, prec = self.criterion_I_s(outputs_pool_s, att_label)
        loss_t += loss_t_s

        loss_id = loss_id + self.criterion_z(outputs_s, att_label)
        # loss_id = loss_id + 0.1 * self.criterion_z(g_att_s, att_label)
        #
        loss_att = 0
